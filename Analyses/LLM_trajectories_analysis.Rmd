...
ggsave("./Output/boxplot_word_count.png", dpi = 600, width = 8, height = 5)
...
```{r}
# create new variable named chunk_max, which is the highest chunk id for a given question/subject
chunks_long <- chunks_long %>%
  group_by(subject_id, question) %>%
  mutate(chunk_max = max(chunk_id)) %>%
  ungroup()


chunks_long_summary <- chunks_long %>%
  group_by(chunk_max, deliberation_function) %>%
  summarise(
...
# Plot mean score with standard error ribbon
ggplot(chunks_long_summary, aes(x = chunk_max, y = mean_score, color = deliberation_function, fill = deliberation_function)) +
  ...
  labs(
    x = "Total Number of Chunks",
    y = "Mean Score (± SE)",
    color = "Deliberation Function",
    fill = "Deliberation Function"
  )

# now per accuracy using facet_wrap
chunks_long_summary <- chunks_long %>%
  group_by(chunk_max, deliberation_function, response) %>%
  summarise(
    mean_score = mean(score, na.rm = TRUE),
    se_score = sd(score, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
...
```
*Note*: as we saw from the chunk distributions, after ~8-10 chunks, it is not really reliable (since there are so few of them).
...
# --------------------------------------------------------------------
# Pivot Wider: Separate Correct and Incorrect
# --------------------------------------------------------------------
trajectory_diff <- mean_trajectory_resp %>%
  filter(response %in% c("Incorrect", "Correct")) %>%
  pivot_wider(
    names_from = response,
    values_from = score
  ) %>%
  mutate(difference = `Correct` - `Incorrect`)
...
p_diff <- ggplot(smoothed_diff, aes(x = normalized_position, y = difference, color = deliberation_function)) +
  ...
  annotate("text", x = 0.05, y = max(smoothed_diff$difference, na.rm = TRUE) * 0.93, 
           label = "Higher for Correct", hjust = 0, size = 3) +
  annotate("text", x = 0.05, y = min(smoothed_diff$difference, na.rm = TRUE) * 0.93, 
           label = "Higher for Incorrect", hjust = 0, size = 3) +
  theme_apa() +
  coord_cartesian(ylim = c(-25, 30)) +
  labs(
    x = "Normalized Position in Text",
    y = "Difference in Mean Score (Correct - Incorrect)",
    color = "Deliberation Function"
  )
...
# ----------------------------------------------------------------------------
gam_model <- bam(
  score ~ accuracy_high * deliberation_function + response + 
    s(normalized_position, by = interaction(accuracy_high, deliberation_function), k = 10) +
    s(subject_id, bs = "re") +
    s(question, bs = "re"),
  data = chunks_long,
  method = "fREML"
...
# Add dummy subject_id, question, and set response to a constant (to control for it)
newdata$subject_id <- levels(chunks_long$subject_id)[1]
newdata$question   <- levels(chunks_long$question)[1]
newdata$response   <- levels(chunks_long$response)[1]  # e.g., always "Correct"
